...
# # # print('**** Enhancement: mean=%.3f std=%.3f' % (mean(GAN2_acc)*100 - mean(cnn2_acc)*100, std(GAN2_acc)*100 - std(cnn2_acc)*100))
    ...
    # def define_generator(latent_dim=200):
    ...
    #     model.add(LeakyReLU(alpha=0.2))
    #     # upsample to 16x16
    #     model.add(Conv2DTranspose(512, (4,4), strides=2, padding='same'))
    #     model.add(LeakyReLU(alpha=0.2))
    #     # upsample to 32x32
    ...
    # # model.summary()
    # # # plot the model
    # # plot_model(model, to_file='final_90/{0} GAN_results/fold_{}/GAN_discriminator_plot.png'.format(subject), show_shapes=True, show_layer_names=True)
    # # # define the generator model
    # # model = define_generator()
    # # # summarize the model
    # # model.summary()
    # # # plot the model
    # # plot_model(model, to_file='final_90/{0} GAN_results/fold_{}/GAN_generator_plot.png'.format(subject), show_shapes=True, show_layer_names=True)
    ...
    #     # save plot to file
    #     pyplot.savefig('final_90/{0} GAN_results/fold_{1}/plots {0}/test_GAN_batch{2}_cl{3}_{4}.png' .format(subject,nfolds, n_batch, cl, epoch+1))
    #     pyplot.close()
    ...
    #     g_model.save('final_90/{0} GAN_results/fold_{1}/models {0}/test_GAN_batch{2}_cl{3}_{4}.h5'.format(subject,nfolds, n_batch, cl, epoch+1))
    ...
    #     # save plot to file
    #     pyplot.grid()
    #     pyplot.savefig('final_90/{0} GAN_results/fold_{1}/test_plot_line_GAN {2}_cl{3}_loss.png' .format(subject,nfolds, n_batch, cl))
    #     pyplot.close()
...
#%% ============================= Genrator =====================================
    ...
    # makedirs('final_90/{0} GAN_results/fold_{1}/GAN_dataset_test0/CL{2}'.format(subject,nfolds, cl), exist_ok=True) 
    ...
    #         # plot raw pixel data
    #         plt.imshow(examples[i])
    #         plt.savefig('final_90/{0} GAN_results/fold_{1}/GAN_dataset_test0/CL{2}/cl{2}_t{3}.jpg'.format(subject,nfolds , cl , i), bbox_inches= 'tight', pad_inches= 0)
    #         plt.close()
    ...
    # ======================= GAN DATA ============================
    gan_img_data, gan_class_name =create_dataset(GAN_data)
    gan_target_dict={s: v for v, s in enumerate(np.unique(gan_class_name))}
    gan_target_dict
    gan_target_val=  [gan_target_dict[gan_class_name[i]] for i in range(len(gan_class_name))]
    x_gan =tf.cast(np.array(gan_img_data), tf.float64).numpy()
    y_gan =tf.cast(list(map(int,gan_target_val)),tf.int32).numpy()
    ...
    # model_training(x_train, y_train,'final_90/{0} GAN_results/fold_{2}/CNN_GAN300/GAN_CNN2 models/{0}_GAN_{1}_CNN2_model.h5'.format( subject, Ad_times, nfolds), create_cnn2(), '{1}_GAN_CNN2 Model accuracy fold {2} --> AD={0} \n'.format( Ad_times, subject, nfolds))        
    # model_training(x_train, y_train,'final_90/{0} GAN_results/fold_{2}/CNN_GAN300/GAN_CNN models/{0}_GAN_{1}_CNN_model.h5'.format( subject, Ad_times, nfolds), create_cnn(), '{1}_GAN_CNN Model accuracy fold {2}--> AD={0} \n'.format( Ad_times, subject, nfolds))    
...
# for subject in subjects:
    ...
    def CNN_GAN_test(cnn=2):
        ...
        for f in range(0,10):
            if cnn == 2 :
            # load the saved model
                model = load_model('final_90/CNN_GAN//{0}_GAN_{1}_CNN2_model_f{2}.h5'.format( subject, Ad_times, f))
                test_loss, test_acc= model.evaluate(testX[f],testY[f],verbose=0)
                print('GAN2 Test Accuracy',test_acc)
                # model = load_model('final_90/{0} GAN_results/fold_{2}/CNN_GAN/GAN_CNN2 models/{0}_GAN_{1}_CNN2_model.h5'.format( subject, Ad_times, f))
                # _, test_acc2= model.evaluate(testX[f],testY[f],verbose=0)
                # print('Test: ',f,' fold Accuracy',test_acc2)
                # if (test_acc2 > test_acc):
                #     model.save('final_90/CNN_GAN/{0}_GAN_{1}_CNN2_model_f{2}.h5'.format( subject, Ad_times, f))
                #     scores.append(test_acc2)
                # else:
                #     model = load_model('final_90/{0} GAN_results/fold_{2}/CNN_GAN300/GAN_CNN2 models/{0}_GAN_{1}_CNN2_model.h5'.format( subject, Ad_times, f))
                #     model.save('final_90/CNN_GAN//{0}_GAN_{1}_CNN2_model_f{2}.h5'.format( subject, Ad_times, f))
                scores.append(test_acc)
                ...
                model = load_model('final_90/{0} GAN_results/fold_{2}/CNN_GAN/GAN_CNN2 models/{0}_GAN_{1}_CNN2_model.h5'.format( subject, Ad_times, f))
                test_loss, test_acc= model.evaluate(testX[f],testY[f],verbose=0)
                print('GAN400 Test Accuracy',test_acc)
                scores.append(test_acc)
            else:
                model = load_model('{1}/fixed_dropout/CNN2 models/{1}_CNN_modelfold{0}_.h5'.format(f,subject))
                test_loss, test_acc= model.evaluate(testX[f],testY[f],verbose=0)
                print('Test Accuracy',test_acc)
                scores.append(test_acc)   
        print('\n >>>> {0} Accuracy: mean={1} std={2}, n={3}' .format (subject, mean(scores)*100, std(scores)*100, len(scores)))
    ...
    # sub_cnn_acc = CNN_GAN_test(cnn=3)
    # total_gan_acc.append(sub_gan_acc)
    sub_gan2_acc = CNN_GAN_test(cnn=2)
    # sub_gan2_acc = CNN_GAN_test(cnn=1)
    total_gan2_acc.append(sub_gan2_acc)
